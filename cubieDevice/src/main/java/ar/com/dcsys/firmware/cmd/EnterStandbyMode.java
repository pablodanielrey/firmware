...
public class EnterStandbyMode {
	
	public interface EnterStandbyResult {
		public void onSuccess();
		public void onFailure();
	}
	
	private static final Logger logger = Logger.getLogger(EnterStandbyMode.class.getName());
	...
	private void checkPreconditions(CamabioResponse rsp) throws CmdException {
		...
		if (rsp.rcm != CamabioUtils.CMD_FP_CANCEL && rsp.rcm != CamabioUtils.CMD_ENTER_STAND_BY) {
			throw new CmdException("RCM != " + this.getClass().getName());
		}
	}
	
	
	public void execute(SerialDevice sd, EnterStandbyResult result) throws CmdException {
		
		MutualExclusion.using[MutualExclusion.SERIAL_DEVICE].acquireUninterruptibly();
		try {
			byte[] cmd = CamabioUtils.enterStandByMode();
			sd.writeBytes(cmd);
